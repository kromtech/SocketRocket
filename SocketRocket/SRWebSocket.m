...
- (void)didConnect;
{
    ...
    int result __attribute__((unused)) = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
	assert(result == 0);
...
- (void)_scheduleCleanup
{
    @synchronized(self) {
        ...
        NSTimer *timer = [NSTimer timerWithTimeInterval:0.f target:self
...
{
    @synchronized(self) {
        // Remove the delegates for each stream so we don't fire any events on close or error
        _inputStream.delegate = nil;
        _outputStream.delegate = nil;
        
        // Close the streams, which will immediately remove them from the run loop.
        [_inputStream close];
        [_outputStream close];
    }
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        int result __attribute__((unused)) = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
		assert(result == 0);
